...
app = Flask(__name__)
app.secret_key = 'pulsekey-medical-secure-key-2024-v1'
...
class QRGenerator:
    ...
    def generate_medical_qr(self, report_data, user_id, report_id):
        ...
        try:
            ...
            return qr_path, qr_filename
            
        except ImportError:
            print("❌ qrcode module not available")
            fallback_path = os.path.join(self.upload_folder, f"medical_qr_{report_id}.png")
            return fallback_path, f"medical_qr_{report_id}.png"
        except Exception as e:
...
def is_recent(date_string):
    ...
    try:
        if isinstance(date_string, str):
            report_date = datetime.strptime(date_string, '%Y-%m-%d %H:%M:%S')
...
def view_report(report_id):
    ...
    try:
        ...
        qr_exists = report.get('qr_code_path') and os.path.exists(report.get('qr_code_path'))
...
@app.route('/generate_qr/<report_id>')
...
def generate_qr_batch():
    ...
    try:
        ...
        return render_template('batch_qr.html', reports=user_reports)
    except Exception as e:
...
def reports():
    ...
    try:
        ...
        return render_template('reports.html', user_reports=user_reports)
    except Exception as e:
